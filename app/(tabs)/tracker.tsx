...
import { formatDateToISO, getWeekStartDate, getWeekEndDate, isDateInPast, addWeeksToDate, subWeeksFromDate } from '@/utils/dateHelpers';
...
export default function VisualAttendanceTracker() {
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      <NameDayEventModal
        isVisible={dayEventToName !== null}
        dayEvent={dayEventToName}
        onClose={() => setDayEventToName(null)}
        onSave={(name, type, date) => {
          if (type === 'holiday') {
            addHoliday({ id: Date.now().toString(), name, startDate: date, endDate: date });
          } else {
            addSkipDay({ id: Date.now().toString(), date, reason: name });
          }
